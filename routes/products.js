...
router.get('/category_id/:categoryId/product_id/:productId', (req, res, next) => {
  connection.query('select * from prdouct where (id = ? or ? = 0) and (category = ? or ?  = 0 )', [req.params.productId, req.params.productId, req.params.categoryId, req.params.categoryId], function(error, results, fields) {
